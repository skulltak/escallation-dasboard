...
app.get('/api/info', (req, res) => res.json({ version: 'v4.8.0', limit: '50mb', db_status: mongoose.connection.readyState, time: new Date().toISOString() }));
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`🚀 Escalation Dashboard v4.8.0 - 3D PRO Live on port ${PORT}`);
        });
    } catch (err) {
        console.error('❌ MongoDB Connection Error:', err.message);
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`🚀 Server running on port ${PORT} (v4.8.0 - DB Offline)`);
        });
    }
};
