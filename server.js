...
app.get('/api/info', (req, res) => res.json({ version: 'v4.5.0', limit: '50mb', db_status: mongoose.connection.readyState, time: new Date().toISOString() }));
...
app.use((req, res, next) => {
    ...
    res.sendFile(indexPath, (err) => {
        if (err) {
            console.error(`SPA Error (v4.5.0): File exists? ${exists}. Path: ${indexPath}`, err);
            res.status(404).send(`[Escalation Dashboard v4.5.0] Deployment Sync Error: Frontend files missing at ${indexPath}. Build check: ${exists}. Please ensure the build command succeeded.`);
        }
    });
});
...
const startServer = async () => {
    try {
        ...
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`🚀 Escalation Dashboard v4.5.0 - 3D PRO Live on port ${PORT}`);
        });
    } catch (err) {
        console.error('❌ MongoDB Connection Error:', err.message);
        app.listen(PORT, '0.0.0.0', () => {
            console.log(`🚀 Server running on port ${PORT} (v4.5.0 - DB Offline)`);
        });
    }
};
